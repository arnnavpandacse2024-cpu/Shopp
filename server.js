...
app.post('/api/state', async (req, res) => {
  try {
    ...
    if (offers && Array.isArray(offers) && offers.length > 0) {
      const offerOps = offers.map(offer => ({
        updateOne: {
          filter: { id: offer.id },
          update: { $set: offer },
          upsert: true
      ...
      await Offer.bulkWrite(offerOps);
    }

    // UPSERT customers
    if (customers && Array.isArray(customers) && customers.length > 0) {
      const custOps = customers.map(cust => ({
        updateOne: {
          filter: { phone: cust.phone },
          update: { $set: cust },
          upsert: true
        }
      }));
      await Customer.bulkWrite(custOps);
    }
...
// DELETE: Remove an active offer
app.delete('/api/offers/:id', async (req, res) => {
  try {
    const result = await Offer.deleteOne({ id: req.params.id });
    if (result.deletedCount === 0) {
      return res.status(404).json({ error: 'Offer not found' });
    }
    console.log(`Deleted offer: ${req.params.id}`);
    io.emit('state_updated');
